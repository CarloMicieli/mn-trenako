...
public class CreateNewScalesController {
    @Post
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON_PROBLEM)
    public HttpResponse<Void> handle(@Body ScaleRequest scaleRequest) {
        ScaleId scaleId = new ScaleId(scaleRequest.name());
        return HttpResponse.created(URI.create(CatalogApis.API_SCALES + "/" + scaleId));
    }
}
